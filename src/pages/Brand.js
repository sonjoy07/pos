...
import { getBrand, showBrand, addBrand, editBrand, deleteBran } from '../reducers/brandReducer'
// mock
...
export default function Brand() {
  ...
  const categories = useSelector(showBrand);
  ...
  const saveBrand = async (values) => {
    const saveData = await dispatch(addBrand(values))
    setOpen(false)
    toast.success(saveData.data.message);
  }
  const updateBrand = async (values) => {
    const updateData = await dispatch(editBrand(values))
    setOpen(false)
    toast.success(updateData.data.message);
  }
  const deleteRow = async (values) => {
    const deleteData = await dispatch(deleteBran(values))
    setOpen(false)
  ...
  const filteredUsers = brandList.length > 0 && applySortFilter(brandList, getComparator(order, orderBy), filterName);
    ...
    <Page title="Brand">
      ...
      >
        <Box sx={{ ...style, width: '60%' }}>
          <h2 id="parent-modal-title">Brand Form</h2>
          {/* <Box
            sx={{
              maxWidth: '100%',
              marginTop: '20px'
